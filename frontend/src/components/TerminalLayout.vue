...
const emit = defineEmits<{
  (e: 'split-pane', paneId: string, orientation: 'horizontal' | 'vertical'): void;
  (e: 'close-pane', paneId: string): void;
  (e: 'pane-initialized', paneId: string, sessionId: string): void;
  (e: 'focus-pane', paneId: string): void;
  (e: 'move-pane', sourceId: string, targetId: string, position: 'left' | 'right' | 'top' | 'bottom' | 'swap'): void;
...
const containerStyle = computed(() => {
  ...
  return {
    ...
    width: '100%',
    height: '100%',
    flexDirection: (props.node.orientation === 'horizontal' ? 'row' : 'column') as any,
    overflow: 'hidden',
  };
});
...
function startResize(e: MouseEvent, index: number) {
  e.preventDefault();
  if (!props.node.children || !props.node.sizes || !layoutContainer.value) return;
...
function onDrop(e: DragEvent, zone: 'left' | 'right' | 'top' | 'bottom' | 'swap') {
  e.preventDefault();
  dragCounter.value = 0;
  dragOverActive.value = false;
  activeDropZone.value = null;
...
<template>
  ...
  <div v-if="node.type === 'split'" ref="layoutContainer" :style="containerStyle" class="split-parent">
    <template v-for="(child, index) in node.children" :key="child.id">
      <TerminalLayout
        ...
        @pane-initialized="(pId, sId) => emit('pane-initialized', pId, sId)"
        @focus-pane="(pId) => emit('focus-pane', pId)"
        @move-pane="(src, tgt, pos) => emit('move-pane', src, tgt, pos)"
        @update-sizes="(nodeId, sizes) => emit('update-sizes', nodeId, sizes)"
      />
